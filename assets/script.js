...
function applyTheme(theme, animate){
  ...
  if(animate){
    ...
    requestAnimationFrame(()=>requestAnimationFrame(()=>html.classList.add('theme-ready')));
  }
...
function toggleTheme(){
  ...
  showToast(document.documentElement.getAttribute('data-theme')==='light'
    ? '☀️ Light mode aktif' : '🌙 Dark mode aktif');
}
...
function doTrace(){
  if(cur<steps.length){
    ...
    requestAnimationFrame(()=>requestAnimationFrame(()=>{
      while(cur<steps.length){ const s=steps[cur]; fillCell(s); cur++; }
...
async function dlMatrixImg(){
  ...
  triggerDl(await capture($('matrix-capture'), isLight ? '#ffffff' : '#161f30'), 'dp_matrix.png');
}
...
async function dlResultImg(){
  ...
  await new Promise(r=>requestAnimationFrame(r));
  const isLight = document.documentElement.getAttribute('data-theme')==='light';
  triggerDl(await capture(clone, isLight ? '#ffffff' : '#161f30'),'alignment_result.png');
...
function buildStyledXLSX(sheetName, cells, merges, colWidths){
  ...
  if(colWidths&&colWidths.length){
    colsXml='<cols>'+colWidths.map(({col,width})=>`<col min="${col}" max="${col}" width="${width}" customWidth="1"/>`).join('')+'</cols>';
  }
